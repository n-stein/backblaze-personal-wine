...
cp $WINEPREFIX/drive_c/Program\ Files\ \(x86\)/Backblaze/bzbui.exe.aside $WINEPREFIX/drive_c/Program\ Files\ \(x86\)/Backblaze/bzbui.exe
until [ -f $WINEPREFIX/drive_c/Program\ Files\ \(x86\)/Backblaze/bzbui.exe ]; do
	echo "Backblaze not installed"
	echo "Initializing the wine prefix"
	WINEARCH=win64 wine64 wineboot -i -u
	configure_wine
...
if [ -f $WINEPREFIX/drive_c/Program\ Files\ \(x86\)/Backblaze/bzbui.exe ]; then
	...
	wine64 $WINEPREFIX/drive_c/Program\ Files\ \(x86\)/Backblaze/bzbui.exe -noqiet
fi
